...
## Источники цен — `/api/sources`

Требует роль `admin`. Каждый источник включается своей переменной окружения:
`FUNPAY_SYNC_ENABLED`, `DIGISELLER_SYNC_ENABLED`, `PLAYEROK_SYNC_ENABLED`.

| Метод | Путь | Комментарий |
|---|---|---|
| GET | `/` | Подключённые источники: код, название, подсказка для поиска каталога |
| GET | `/:provider/games` | Каталог источника: `?q=` — запрос, `?refresh=true` — обновить кэш |
...
| POST | `/saved/:id/active` | Включить/выключить |
| DELETE | `/saved/:id` | Убрать |

Что делает `POST /:provider/sync` за один вызов:

1. читает раздел источника и разбирает предложения;
2. группирует их в варианты — по фильтрам площадки или по правилам из названия;
3. пишет срез рыночных цен (минимум, медиана, среднее, максимум, число продавцов),
...
6. пересчитывает агрегаты варианта и пишет историю цен.

Особенности источников:

| Источник | Каталог | Ограничения |
|---|---|---|
| `funpay` | Поиск по названию игры, кэш на сутки | Валюта фиксируется cookie `cy` |
| `digiseller` | По ID продавца, ID товара или ссылке с plati.market | Магазин = один поставщик; имя продавца берётся из карточки товара |
| `playerok` | По slug игры со страницы площадки | Не больше 24 позиций за запрос; за проход читается ~1000 позиций, признак `truncated`; блокирует IP дата-центров — нужен `PLAYEROK_PROXY_URL` |
